...
func EstablishDBClient() *DB_Client {
	...
	DB_USER, userError := utils.GetEnvVariable("MONGOUSER")

	DB_PASSWD, passwdError := utils.GetEnvVariable(("MONGOPASSWORD"))
	...
	if passwdError != nil {
		log.Fatal(passwdError)
	}

	var cred options.Credential

	cred.Username = DB_USER
	cred.Password = DB_PASSWD

	// MongoDB URI (connect to your local MongoDB instance)
	clientOptions := options.Client().ApplyURI("mongodb://" + DB_HOST + ":" + DB_PORT).SetAuth(cred)
